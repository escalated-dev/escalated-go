...
type SLAPolicy struct {
	...
	FirstResponseHours JSONText `json:"first_response_hours"`
	// ResolutionHours is a JSON map of priority -> hours, e.g. {"low":72,"medium":24,"high":8}
	ResolutionHours JSONText `json:"resolution_hours"`
...
func hoursFor(raw JSONText, priority string) (float64, bool) {
	if len(raw) == 0 {
		return 0, false
	}
